...
describe('git()', () => {
  ...
  it('builds the correct git command string', () => {
    let capturedCmd;
    const git = freshGit((cmd) => { capturedCmd = cmd; return ''; });
    git('rev-parse HEAD');
    assert.equal(capturedCmd, 'git rev-parse HEAD');
  });

  it('uses process.cwd() as the default cwd option', () => {
    let capturedOpts;
    const git = freshGit((_cmd, opts) => { capturedOpts = opts; return ''; });
    git('status');
  ...
  it('throws with trimmed err.stderr when execSync throws and stderr is set', () => {
    const git = freshGit(() => {
  ...
  it('throws with err.message when execSync throws and stderr is empty', () => {
    const git = freshGit(() => {
  ...
  it('throws with err.message when execSync throws and stderr is null', () => {
    const git = freshGit(() => {
  ...
  it('throws with err.message when execSync throws and stderr is undefined', () => {
    const git = freshGit(() => {
